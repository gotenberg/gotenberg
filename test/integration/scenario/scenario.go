...
func (s *scenario) iHaveADefaultGotenbergContainer(ctx context.Context) (context.Context, error) {
	c, err := startGotenbergContainer(ctx, s.hostPort.use(), nil)
	if err != nil {
...
func (s *scenario) iHaveAGotenbergContainerWithTheFollowingEnvironmentVariables(ctx context.Context, envTable *godog.Table) (context.Context, error) {
	...
	c, err := startGotenbergContainer(ctx, s.hostPort.use(), env)
	if err != nil {
...
func (s *scenario) iHaveAServer(ctx context.Context) error {
	...
	s.server = srv
	s.server.start(ctx, s.hostPort.use())
	return nil
}
...
func (s *scenario) theHeaderValueShouldBe(kind, name, expected string) error {
	var actual string
	if kind == "response" {
		...
		return errors.New("no wehbook request found")
	} else {
...
func (s *scenario) thePdfShouldHavePages(ctx context.Context, name string, pages int) error {
	...
	expected, err := strconv.Atoi(matches[1])
	if err != nil {
		return fmt.Errorf("convert pages value %q to integer: %w", matches[1], err)
	}

	if expected != pages {
		return fmt.Errorf("expected %d pages, but actual is %d", expected, pages)
	}
...
func (s *scenario) thePdfShouldHaveTheFollowingContentAtPage(ctx context.Context, name string, page int, expected *godog.DocString) error {
	path := fmt.Sprintf("%s/%s/%s", s.workdir, s.resp.Header().Get("Gotenberg-Trace"), name)
...
func InitializeScenario(ctx *godog.ScenarioContext) {
	...
	ctx.Before(func(ctx context.Context, sc *godog.Scenario) (context.Context, error) {
		p, err := freePort()
		if err != nil {
			return ctx, fmt.Errorf("get free port: %w", err)
		}
		s.hostPort = p
		return ctx, nil
	})
	...
	ctx.Then(`^the (response|webhook request|file request) header "([^"]*)" should be "([^"]*)"$`, s.theHeaderValueShouldBe)
	...
	ctx.Then(`^the "([^"]*)" PDF should have the following content at page (\d+):$`, s.thePdfShouldHaveTheFollowingContentAtPage)
