...
func startGotenbergContainer(ctx context.Context, env map[string]string) (*testcontainers.DockerNetwork, testcontainers.Container, error) {
	...
	// For non-Linux, create a dedicated network.
	...
	if runtime.GOOS == "linux" {
		// Wait for a log message indicating the server is ready.
		waitStrategy = wait.ForLog("api: server started").WithStartupTimeout(2 * time.Minute)
	} else {
		waitStrategy = wait.ForListeningPort("3000/tcp")
	}
	...
	req := testcontainers.ContainerRequest{
		...
		// Use the dedicated network only when not on Linux.
		Networks:   []string{},
		WaitingFor: waitStrategy,
		Env:        env,
	}

	// Only set the Networks field if networkName is available.
	if n != nil {
		req.Networks = []string{n.Name}
	}
