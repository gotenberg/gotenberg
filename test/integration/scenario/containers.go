...
func startGotenbergContainer(ctx context.Context, hostPort int, env map[string]string) (testcontainers.Container, error) {
	...
	req := testcontainers.ContainerRequest{
		...
		HostConfigModifier: func(hostConfig *container.HostConfig) {
			...
			hostConfig.PortBindings = nat.PortMap{
				nat.Port(fmt.Sprintf("%d/tcp", hostPort)): []nat.PortBinding{
					{
						HostIP:   "0.0.0.0",
