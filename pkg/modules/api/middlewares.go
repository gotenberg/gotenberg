...
//
// Logic for determining the Trace ID:
// 1. If an OpenTelemetry header is present (Remote Span), that ID is used.
// 2. If no OTel header, but the legacy specific `header` is present, that value is used.
// 3. Otherwise, the ID generated by the new OpenTelemetry span is used.
//
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if spanCtx.IsRemote() {
				...
				legacyTrace := c.Request().Header.Get(header)
				if legacyTrace != "" {
					traceId = legacyTrace
				} else {
					if spanCtx.IsValid() {
			...
			tracer.Inject(ctx, c.Response().Header())

			c.Set("trace", traceId)
			c.Set("traceHeader", header)
			c.Response().Header().Add(header, traceId)
