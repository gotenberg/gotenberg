...
func TestFormDataPdfBulkStamps(t *testing.T) {
	...
	t.Run("rejects missing expression for text source", func(t *testing.T) {
		...
		want := "Invalid form data: form field 'stamps' is invalid (got '[{\"source\":\"text\"}]', resulting to entry 0 requires a non-empty expression for text source)"
		if err.Error() != want {
	...
	t.Run("rejects missing file for image source", func(t *testing.T) {
		...
		want := "Invalid form data: form field 'stamps' is invalid (got '[{\"source\":\"image\"}]', resulting to entry 0 requires a non-empty file for image or pdf source)"
		if err.Error() != want {
			t.Fatalf("error = %q, want %q", err.Error(), want)
		}
	})
