...
func (mod *PdfEngines) Descriptor() gotenberg.ModuleDescriptor {
	return gotenberg.ModuleDescriptor{
		...
		FlagSet: func() *flag.FlagSet {
			...
			fs.StringSlice("pdfengines-password-engines", []string{"qpdf", "pdftk", "pdfcpu", "libreoffice-pdfengine"}, "Set the PDF engines and their order for the password protection feature - empty means all")
...
func (mod *PdfEngines) Provision(ctx *gotenberg.Context) error {
	...
	passwordNames := flags.MustStringSlice("pdfengines-password-engines")
	...
	mod.passwordNames = defaultNames
	if len(passwordNames) > 0 {
		mod.passwordNames = passwordNames
	}
...
func (mod *PdfEngines) Validate() error {
	...
	findNonExistingEngines(mod.passwordNames)
...
func (mod *PdfEngines) SystemMessages() []string {
	return []string{
		...
		fmt.Sprintf("password protection engines - %s", strings.Join(mod.passwordNames[:], " ")),
	}
}
...
func (mod *PdfEngines) Routes() ([]api.Route, error) {
	...
	return []api.Route{
		...
		passwordProtectionRoute(engine),
	}, nil
}
