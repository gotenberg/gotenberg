...
func (b *chromiumBrowser) pdf(ctx context.Context, logger *slog.Logger, url, outputPath string, options PdfOptions) error {
	...
	return b.do(ctx, logger, url, options.Options, chromedp.Tasks{
		...
		navigateActionFunc(logger, url, navigateOptions{
			SkipNetworkIdleEvent:       options.SkipNetworkIdleEvent,
			SkipNetworkAlmostIdleEvent: options.SkipNetworkAlmostIdleEvent,
			SkipLifecycleEvents:        options.WaitForExpression != "" || options.WaitForSelector != "",
		}),
