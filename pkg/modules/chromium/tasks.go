...
func injectRobDetectDefenceScriptFunc(logger *zap.Logger, disableJavaScript bool, customScriptUrl string) chromedp.ActionFunc {
	return func(ctx context.Context) error {
		if disableJavaScript {
			logger.Debug("Action `inject robot detect defense script` is disabled by disableJavaScript option.")
			return nil
		}
		logger.Debug(fmt.Sprintf("Action `inject robot detect defense script` is start."))
		var script string
		if customScriptUrl != "" {
			script = fmt.Sprintf(`document.body.appendChild(Object.assign(document.createElement('script'), {src: %s))`, customScriptUrl)
			logger.Info(fmt.Sprintf("Use custom stealth.js url: %s", customScriptUrl))
		} else {
			// generated by https://github.com/berstend/puppeteer-extra/tree/master/packages/extract-stealth-evasions
			bs, err := os.ReadFile("./resources/js/stealth.min.js")
			if err != nil {
				// if read local stealth.min.js failed, just skip it, this action doesn't matter result.
				logger.Error(fmt.Sprintf("Read local stealth.min.js failed with error: %s", err.Error()), zap.Error(err))
				return nil
			}
			script = string(bs)
			logger.Info("Use default stealth.min.js")
		}
		id, err := page.AddScriptToEvaluateOnNewDocument(script).Do(ctx)
		if err != nil {
			logger.Error(fmt.Sprintf("Inject stealth.min.js to page failed with error: %s", err.Error()), zap.Error(err))
		} else {
			logger.Info(fmt.Sprintf("Inject stealth.min.js to page succeed, script id: %s", id))
		}
		return nil
	}
}
...
func waitForExpressionBeforePrintActionFunc(logger *zap.Logger, disableJavaScript bool, expression string) chromedp.ActionFunc {
	return func(ctx context.Context) error {
		...
		for {
			...
			case <-ticker.C:
				var ok bool
				err := chromedp.Evaluate(expression, &ok).Do(ctx)
				if err != nil {
