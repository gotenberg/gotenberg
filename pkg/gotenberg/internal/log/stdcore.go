...
// NewStdCore returns a [zapcore.Core] instance for the standard output.
func NewStdCore(level zapcore.Level, format string, fieldsPrefix string, enableGcpFields bool) (zapcore.Core, error) {
	encoder, err := newLogEncoder(format, enableGcpFields)
	if err != nil {
		return nil, fmt.Errorf("get log encoder: %w", err)
	}

	return rootCore{Core: zapcore.NewCore(encoder, os.Stderr, level), level: level, fieldsPrefix: fieldsPrefix}, nil
}
