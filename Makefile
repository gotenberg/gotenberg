...
build-alpine: ## Build the Gotenberg's Docker alpine image
	...
	--build-arg PDFTK_VERSION=$(PDFTK_VERSION) \
	--build-arg PDFCPU_VERSION=$(PDFCPU_VERSION) \
	-t $(DOCKER_REGISTRY)/$(DOCKER_REPOSITORY):$(GOTENBERG_VERSION)-$(ALPINE_IMAGE_SUFFIX) \
	-f $(DOCKERFILE_ALPINE) $(DOCKER_BUILD_CONTEXT)
...
run-alpine: ## Start a Gotenberg alpine container
	...
	-e GOTENBERG_API_BASIC_AUTH_USERNAME=$(GOTENBERG_API_BASIC_AUTH_USERNAME) \
	-e GOTENBERG_API_BASIC_AUTH_PASSWORD=$(GOTENBERG_API_BASIC_AUTH_PASSWORD) \
	$(DOCKER_REGISTRY)/$(DOCKER_REPOSITORY):$(GOTENBERG_VERSION)-$(ALPINE_IMAGE_SUFFIX) \
	gotenberg \
	--gotenberg-graceful-shutdown-duration=$(GOTENBERG_GRACEFUL_SHUTDOWN_DURATION) \
	--api-port=$(API_PORT) \
	--api-port-from-env=$(API_PORT_FROM_ENV) \
